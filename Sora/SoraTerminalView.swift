...
/// Sora's libghostty backend: Ghostty's Metal-backed terminal surface plus
/// Sora's pane focus, context menu, effective application focus, and
/// Finder/file-tree drop behavior.
///
/// This is the only type in Sora that knows libghostty exists. It owns the
/// `TerminalController`, renders Sora's settings into Ghostty's config, and
/// translates Ghostty's delegate callbacks into ``TerminalBackendEvents`` —
/// see `SoraTerminalView+Ghostty.swift`.
final class SoraTerminalView: AppTerminalView, TerminalBackendSurface {
    ...
    /// Mirrors Sora's OSC 9;4 indicator: a two-point bar
    ...
    /// The terminal is effectively focused only while Sora itself is active,
    ...
    /// Sora consistently reserves right-click for its terminal/pane menu. This
    /// matches Sora's existing UI, including focusing before Paste.
...
final class SoraTerminalProgressBarView: NSView {
    private let trackLayer = CALayer()
    private let barLayer = CALayer()
    private let indeterminateAnimationKey = "soraTerminalProgressIndeterminate"
