...
/// Sora's Alacritty backend: a `TerminalBackendSurface` rendered with Metal
...
/// here is Sora's: cell layout, glyph drawing, the cursor, selection, and the
...
final class AlacrittyTerminalView: NSView, TerminalBackendSurface, NSUserInterfaceValidations {
    ...
    /// Matches the window padding Sora's Ghostty panes use, so a pane looks
    ...
    /// Sora is inactive. Retaining one frame is much cheaper than keeping the
    ...
    /// with OSC 22, matching the `.text` default of Sora's Ghostty panes.
    ...
    private let progressBar = SoraTerminalProgressBarView(frame: .zero)
    ...
    deinit {
        ...
        if let handle { sora_alacritty_free(handle) }
    }
    ...
    private func start(launch: TerminalLaunch) {
        ...
        if handle == nil {
            NSLog("sora: failed to start the Alacritty backend for \(launch.program)")
            return
        }
    }
    ...
    /// Sora windows, but release the CAMetalLayer drawable pool and renderer.
    ...
    /// Re-renders the retained image while Sora or its terminal window is not
    ...
    private func addInactiveCursorOverlay(to frozenLayer: CALayer) {
        ...
        var snapshot = SoraSnapshot()
        sora_alacritty_snapshot(handle, &snapshot)
    ...
    private func reportWorkingDirectory() {
        ...
        let pid = sora_alacritty_foreground_pid(handle)
        guard let path = processWorkingDirectory(pid: pid)
            ?? processWorkingDirectory(pid: sora_alacritty_child_pid(handle))
        else { return }
    ...
    func applyAppearance() {
        ...
        var theme = AlacrittyTheme.current()
        if let handle {
            withUnsafePointer(to: &theme) { sora_alacritty_set_theme(handle, $0) }
        }
    ...
    private func renderFrame(
        ...
        if !waitUntilCompleted, sora_alacritty_synchronized_update(handle) {
            return true
        }
        ...
        guard let renderer = metalRenderer else {
            NSLog("sora: no Metal device; the Alacritty backend cannot draw")
            return false
        }
        ...
        var damage = SoraDamage()
        sora_alacritty_take_damage(handle, &damage)
        ...
        var dirtyRows: [Int]?
        if needsUnconditionalRedraw || damage.kind == SORA_DAMAGE_FULL {
            dirtyRows = nil
        } else if damage.kind == SORA_DAMAGE_PARTIAL, let rows = damage.rows {
    ...
    private func applyHoveredURLUnderline(to snapshot: inout SoraSnapshot) {
    ...
    private func updateKittyGraphics(handle: OpaquePointer) {
        var snapshot = SoraKittySnapshot()
        sora_alacritty_kitty_snapshot(handle, &snapshot)
    ...
    private func updateMarkedTextOverlay(snapshot: SoraSnapshot) {
    ...
    /// Feeds Sora's overlay scrollbar.
    ///
    ...
    private func reportScroll() {
        ...
        // scrollback; Sora's scrollbar measures down from the oldest row.
    ...
    func handleEvent(kind: UInt32, payload: Data) {
        if events == nil,
           kind == SORA_EVENT_TITLE
            || kind == SORA_EVENT_BELL
            || kind == SORA_EVENT_EXIT
            || kind == SORA_EVENT_CLIPBOARD_LOAD
            || kind == SORA_EVENT_WORKING_DIRECTORY
            || kind == SORA_EVENT_NOTIFICATION
            || kind == SORA_EVENT_SHELL_PROMPT_START
            || kind == SORA_EVENT_SHELL_COMMAND_START
            || kind == SORA_EVENT_SHELL_COMMAND_EXECUTING
            || kind == SORA_EVENT_SHELL_COMMAND_FINISHED {
        ...
        case SORA_EVENT_CLIPBOARD_LOAD:
        ...
        case SORA_EVENT_WORKING_DIRECTORY:
    ...
    /// cursor keywords and fallbacks as Sora's Ghostty panes, where macOS has
    ...
    private func resolveClipboard(
        ...
        bytes.withUnsafeBufferPointer { pointer in
            sora_alacritty_resolve_clipboard(
    ...
    func scroll(toFraction fraction: Double) {
        ...
        sora_alacritty_scroll_to_offset(handle, history - min(fromTop, history))
        scheduleRender(force: true)
    }

    var hasSelection: Bool {
        guard let handle else { return false }
        return sora_alacritty_has_selection(handle)
    }
    ...
    func findSelection() {
        guard let handle, sora_alacritty_has_selection(handle) else { return }
        let needed = sora_alacritty_selection_text(handle, nil, 0)
        guard needed > 0 else { return }
        var buffer = [UInt8](repeating: 0, count: needed)
        let written = buffer.withUnsafeMutableBufferPointer { pointer in
            sora_alacritty_selection_text(handle, pointer.baseAddress, needed)
        }
    ...
    private func exportFile(scrollbackOnly: Bool) -> String? {
        guard let handle else { return nil }
        let needed = sora_alacritty_buffer_text(handle, scrollbackOnly, nil, 0)
        guard needed > 0 else { return nil }

        var buffer = [UInt8](repeating: 0, count: needed)
        let written = buffer.withUnsafeMutableBufferPointer { pointer in
            sora_alacritty_buffer_text(handle, scrollbackOnly, pointer.baseAddress, needed)
        }
        guard written > 0 else { return nil }

        let manager = FileManager.default
        let directory = manager.temporaryDirectory
            .appendingPathComponent("sora-export-\(UUID().uuidString)", isDirectory: true)
        do {
    ...
    private func write(_ bytes: [UInt8]) {
        ...
        bytes.withUnsafeBufferPointer { pointer in
            sora_alacritty_write(handle, pointer.baseAddress, pointer.count)
        }
        resetCursorBlink()
    }
    ...
    private func writeControl(_ bytes: [UInt8]) {
        guard let handle, !bytes.isEmpty else { return }
        bytes.withUnsafeBufferPointer { pointer in
            sora_alacritty_write_control(handle, pointer.baseAddress, pointer.count)
        }
    }

    private var terminalMode: AlacrittyTerminalMode {
        guard let handle else { return [] }
        return AlacrittyTerminalMode(rawValue: sora_alacritty_mode(handle))
    }
    ...
    override func keyDown(with event: NSEvent) {
        if event.modifierFlags.contains(.command), let handle {
            ...
            case 115: // Command-Home
                ...
                let history = snapshot.total_lines > snapshot.screen_lines
                    ? snapshot.total_lines - snapshot.screen_lines : 0
                sora_alacritty_scroll_to_offset(handle, history)
                scheduleRender(force: true)
                reportScroll()
                return
            case 119: // Command-End
                sora_alacritty_scroll_to_offset(handle, 0)
                scheduleRender(force: true)
                reportScroll()
                return
            case 116, 121: // Command-Page Up / Page Down
                let delta = Int32(max(gridSize.rows, 1)) * (event.keyCode == 116 ? 1 : -1)
                sora_alacritty_scroll(handle, delta)
                scheduleRender(force: true)
                reportScroll()
                return
            default:
        ...
        // or a shortcut Sora's menus own.
    ...
    override func mouseDown(with event: NSEvent) {
        ...
        sora_alacritty_selection_start(
    ...
    override func mouseDragged(with event: NSEvent) {
        ...
        let point = gridPoint(for: event)
        sora_alacritty_selection_update(
    ...
    override func scrollWheel(with event: NSEvent) {
        ...
        sora_alacritty_scroll(handle, Int32(lines))
        scheduleRender(force: true)
        reportScroll()
    }
    ...
    private func url(
        ...
        guard let handle else { return nil }
        let needed = sora_alacritty_url_at(
            handle, Int32(point.line), point.column, nil, nil, 0
        )
        guard needed > 0 else { return nil }
        var buffer = [UInt8](repeating: 0, count: needed)
        var range = SoraURLRange()
        let written = buffer.withUnsafeMutableBufferPointer { pointer in
            sora_alacritty_url_at(
    ...
    override func selectAll(_ sender: Any?) {
        guard let handle else { return }
        sora_alacritty_select_all(handle)
        scheduleRender(force: true)
    }
    ...
    /// Sora reserves right-click for its terminal/pane menu, matching the
...
extension TerminalLaunch {
    /// Builds a `SoraConfig` whose C strings stay alive for the call. Cargo's
    ...
    func withCConfig<T>(
        ...
        cellHeight: UInt16,
        scrollbackLines: Int,
        _ body: (UnsafePointer<SoraConfig>) -> T
    ) -> T {
...
enum AlacrittyTheme {
    /// Sora's active terminal theme in the flat form the bridge resolves
    /// colors against, so Alacritty panes match Ghostty panes exactly.
    static func current() -> SoraTheme {
