...
/// `SORA_MODE_*` bits the Rust bridge reports.
...
/// whatever owns the keyboard, which here is Sora. This covers the xterm
...
enum AlacrittyKeyMap {
    ...
    /// (IME composition, or a shortcut Sora's menus own).
    static func bytes(
        for event: NSEvent,
        ...
        if flags.contains(.command) {
            ...
            // Command shortcuts still fall through to Sora's menus.
