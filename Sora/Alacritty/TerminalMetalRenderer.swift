...
/// Sora's editor and to the Ghostty backend, which is the whole reason the
...
final class TerminalMetalRenderer {
    ...
    init?(device: MTLDevice) {
        ...
        guard let library = try? device.makeLibrary(source: Self.shaderSource, options: nil),
              let vertexFunction = library.makeFunction(name: "sora_terminal_vertex"),
              let fragmentFunction = library.makeFunction(name: "sora_terminal_fragment")
        else { return nil }
    ...
    private func buildRow(
        ...
        for column in 0..<columns {
            let cell = cells[row * columns + column]
            guard cell.flags & UInt16(SORA_CELL_HIDDEN) == 0,
                  cell.flags & UInt16(SORA_CELL_WIDE_SPACER) == 0
            else { continue }
            ...
            if cell.ch != 0,
               cell.text_len > 0 || cell.ch != UInt32(UInt8(ascii: " ")) {
                ...
                let key = TerminalGlyphAtlas.Key(
                    content: content,
                    bold: cell.flags & UInt16(SORA_CELL_BOLD) != 0,
                    italic: cell.flags & UInt16(SORA_CELL_ITALIC) != 0
                )
    ...
    private func blockCursorInsertionIndex(snapshot: SoraSnapshot) -> Int? {
    ...
    vertex VertexOut sora_terminal_vertex(
    ...
    fragment float4 sora_terminal_fragment(
