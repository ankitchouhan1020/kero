...
/// Which terminal emulator draws and drives Sora's panes.
///
/// Both backends implement Sora's own surface vocabulary, keeping emulator
...
enum TerminalBackend: String, CaseIterable, Identifiable, Sendable {
    ...
    /// The backend Sora uses when the config names none, or names one this
    /// build cannot create.
    static let fallback = TerminalBackend.libghostty

    var id: String { rawValue }

    /// Sora's real surface identity, distinct from `TERM_PROGRAM`, which is a
    ...
    /// selection that cannot take effect would misreport what Sora will do.
    ...
    /// Ghostty protocols Sora implements, including OSC 777 notifications and
    ...
    func makeSurface(launch: TerminalLaunch) -> (any TerminalBackendSurface)? {
        switch self {
        case .libghostty:
            return SoraTerminalView(launch: launch)
        case .alacritty:
...
/// Everything a backend needs to start one pane's shell. Sora resolves the
...
/// What Sora needs from a terminal surface, in Sora's vocabulary rather than
/// any one emulator's.
///
...
protocol TerminalBackendSurface: NSView {
    ...
    /// True only while Sora is active, its window is key, and this exact
    ...
    ///
    /// Named for Sora's UI (⌘F, `TerminalFind`) rather than for any backend's
    ...
    /// needle backend-side so Sora never has to read it out and re-escape it.
    func findSelection()
    ...
    /// temporary directory: Sora validates that before reading, and deletes
    ...
    /// viewport alone. Nil when there is none — which is also how Sora tells a
...
protocol TerminalBackendEvents: AnyObject {
    ...
    /// The backend started a find of its own accord — Sora's ⌘E path resolves
...
/// A backend asking Sora to confirm clipboard access before it proceeds.
