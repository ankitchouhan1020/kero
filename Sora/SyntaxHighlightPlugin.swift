...
//  sora
//
//  sora's own STTextView highlighting plugin. It reuses STTextView-Plugin-Neon's
...
//
//  Languages are named by sora's own `SyntaxLanguage`, not the plugin's
...
/// immutable and thread-safe once built, so it's compiled once before that
/// language's initial parse and every later file reuses it.
@MainActor
...
final class SyntaxHighlightCoordinator {
    ...
    init(
        ...
        // Empty fonts table (see SyntaxHighlighting.theme) → this keeps sora's
        ...
        // Install the query before parsing, matching Neon's reliable lifecycle.
        // Deferring this created a race where the initial parse and viewport
        // invalidation finished while the highlighter still had its no-op provider.
        ...
        tsClient.didChangeContent(
            ...
            completionHandler: { [weak self] in self?.highlighter?.invalidate() }
        )
    }
    ...
    /// Install the query before the initial parse. Queries are cached, so only
    /// the first file of each language pays the compilation cost.
    private func installTokenProvider(textContentManager: NSTextContentManager) {
        ...
        do {
            let query = try SwiftTreeSitter.Query(language: tsLanguage, data: highlightsData)
            HighlightQueryCache.store(query, for: language)
            finishInstall(highlightsQuery: query, textContentManager: textContentManager)
        } catch {
            NSLog("Sora syntax highlighting query failed for %@: %@", String(describing: language), String(describing: error))
        }
    }
    ...
    private func setTokenProvider(query: SwiftTreeSitter.Query, textContentManager: NSTextContentManager) {
        // Neon's fully asynchronous path can strand a request while TextKit is
        // relaying out the viewport. Requests are capped at 1,024 characters,
        // so prefer the immediate parser path and fall back only when needed.
        highlighter?.tokenProvider = tsClient.tokenProvider(with: query, executionMode: .synchronousPreferred) { range, _ in
