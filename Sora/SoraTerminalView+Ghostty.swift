//
//  SoraTerminalView+Ghostty.swift
//  sora
//
...
/// The libghostty half of Sora's terminal backend: starting the emulator,
/// rendering Sora's settings into Ghostty's config, and translating Ghostty's
/// delegate callbacks into ``TerminalBackendEvents``.
///
/// Nothing above `SoraTerminalView` names a Ghostty type, so a second backend
/// only has to satisfy ``TerminalBackendSurface`` — it does not have to
/// re-implement any of this.
extension SoraTerminalView {
    // MARK: - Lifecycle
    ...
    private static func terminalConfiguration(command: String) -> TerminalConfiguration {
        ...
        return TerminalConfiguration { builder in
            ...
            // Keep Sora's bundled icon font as a fallback after the selected
            ...
            // Sora's insets around the grid live inside ghostty as
            ...
            // Sora owns the app-level command map. Leaving Ghostty's defaults
            ...
            // reclaiming any shortcut owned by Sora's command menus.
            for keybind in [
                "alt+left=esc:b",
                "alt+right=esc:f",
                "super+left=text:\\x01",
...
extension SoraTerminalView: TerminalSurfaceTitleDelegate {
    func terminalDidChangeTitle(_ title: String) {
        events?.terminalDidChangeTitle(title)
    }
}

extension SoraTerminalView: TerminalSurfacePwdDelegate {
    func terminalDidChangeWorkingDirectory(_ path: String) {
        events?.terminalDidChangeWorkingDirectory(path)
    }
}

extension SoraTerminalView: TerminalSurfaceBellDelegate {
    func terminalDidRingBell() {
        events?.terminalDidRingBell()
    }
}

extension SoraTerminalView: TerminalSurfaceCloseDelegate {
    func terminalDidClose(processAlive: Bool) {
        events?.terminalDidClose(processAlive: processAlive)
    }
}

extension SoraTerminalView: TerminalSurfaceDesktopNotificationDelegate {
    func terminalDidRequestDesktopNotification(title: String, body: String) {
        events?.terminalDidRequestDesktopNotification(title: title, body: body)
    }
}

extension SoraTerminalView: TerminalSurfaceProgressReportDelegate {
...
extension SoraTerminalView: TerminalSurfaceCommandFinishedDelegate {
...
extension SoraTerminalView: TerminalSurfaceOpenURLDelegate {
...
extension SoraTerminalView: TerminalSurfaceHoverLinkDelegate {
    func terminalDidUpdateHoverLink(_ url: String?) {
        hoveredLink = url
    }
}

extension SoraTerminalView: TerminalSurfaceScrollbarDelegate {
...
extension SoraTerminalView: TerminalSurfaceSearchDelegate {
...
extension SoraTerminalView: TerminalSurfaceClipboardConfirmationDelegate {
