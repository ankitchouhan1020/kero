...
struct ContentView: View {
    ...
    var body: some View {
        ...
        .overlay {
            if manager.isSpaceCreatorPresented {
                ZStack {
                    Color.black.opacity(0.32)
                        .ignoresSafeArea()
                    SpaceCreationView(manager: manager)
                }
                .zIndex(20)
            }
        }
    ...
    private var emptyState: some View {
        if manager.selectedProject == nil {
            emptyStatePrompt(
                title: "No open Spaces",
                buttonTitle: "New Space  ⌘N",
                action: { manager.presentSpaceCreator() }
            )
        } else {
...
/// Optional context toolbar. Global navigation lives in the Spaces sidebar;
/// this bar identifies the selected tab and keeps only content-local actions.
private struct MainHeaderView: View {
    ...
    var body: some View {
        HStack(spacing: 8) {
            ChromeIconButton(
                ...
                tooltip: manager.isLeftSidebarVisible ? "Hide Spaces (⌘B)" : "Show Spaces (⌘B)",
            ...
            if let project = manager.selectedProject,
               ...
               let content = tab.focusedContent {
                let title = titleLabel(for: tab, project: project, content: content)
                ...
                HStack(alignment: .firstTextBaseline, spacing: 8) {
                    Text(title)
                        ...
                        .lineLimit(1)
                    if let scope = scopeLabel(for: project, content: content),
                       scope.localizedCaseInsensitiveCompare(title) != .orderedSame {
                        Text(scope)
            ...
            if manager.canFind {
                ChromeIconButton(systemImage: "magnifyingglass", tooltip: "Find (⌘F)") {
                    manager.performFindAction(.show)
                }
            }
            if manager.selectedProject?.canSplit == true {
                ChromeIconButton(systemImage: "rectangle.split.2x1", tooltip: "Split Right (⌘D)") {
                    manager.split(toward: .right)
                }
            }
            if manager.selectedProject?.hasSplitPanes == true {
                ChromeIconButton(
                    systemImage: manager.isPaneZoomed
                        ? "arrow.down.forward.and.arrow.up.backward"
                        : "arrow.up.backward.and.arrow.down.forward",
                    tooltip: manager.isPaneZoomed ? "Exit Pane Zoom (⇧⌘↩)" : "Zoom Pane (⇧⌘↩)"
                ) {
                    manager.togglePaneZoom()
                }
            }
            if manager.selectedProject != nil {
                ChromeIconButton(systemImage: "sidebar.right", tooltip: "Toggle Right Sidebar (⇧⌘B)") {
    ...
    private func titleLabel(for tab: PaneTab, project: Project, content: PaneContent) -> String {
        if let customName = tab.customName, !customName.isEmpty {
            return customName
        }
        if case .session = content, let path = project.selectedSession?.currentDirectoryPath {
            let folder = URL(fileURLWithPath: path, isDirectory: true).lastPathComponent
            return folder.isEmpty ? "/" : folder
        }
        return tab.displayTitle ?? "Untitled"
    }

    private func scopeLabel(for project: Project, content: PaneContent) -> String? {
        switch content {
        case .file(let file):
            return URL(fileURLWithPath: file.path).deletingLastPathComponent().lastPathComponent
        case .diff(let diff):
