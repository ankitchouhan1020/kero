...
enum Theme {
    ...
    /// Sora's built-in themes: the GitHub Default palettes under sora's own
    ...
    nonisolated static let defaultDarkDefinition = soraDefault(
        named: defaultDarkThemeName, from: "GitHub Dark Default", dark: true
    )
    nonisolated static let defaultLightDefinition = soraDefault(
    ...
    /// terminal backends. Each appearance has 29 catalog themes plus Sora's
    ...
    /// Sora's Default comes first; the duplicate GitHub Default catalog rows
    ...
    /// A sora built-in or catalog theme by name.
    ...
    /// bundled `sora +themes` browser uses this while the user moves through
    ...
    /// Whether the selected theme for one appearance is a sora built-in
    /// Default theme, which keeps the sidebar's translucent material.
    nonisolated static func isDefault(dark: Bool) -> Bool {
        terminal(dark: dark).isSoraDefault
    }

    /// A copy of a catalog theme under a sora-owned name.
    private nonisolated static func soraDefault(
...
nonisolated extension GhosttyThemeDefinition {
    /// Whether this is one of sora's built-in Default themes, which keep the
    /// pre-theme chrome (material sidebar, label-based hairlines).
    var isSoraDefault: Bool {
    ...
    func paletteNSColor(at index: Int) -> NSColor? { palette[index].map(Self.nsColor) }
    ...
    /// Sidebar fill: sora's built-in Default themes keep the GitHub
