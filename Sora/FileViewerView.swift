...
final class FileTab: nonisolated ObservableObject, nonisolated Identifiable {
    ...
    var systemImage: String {
        if case .image = content { return "photo" }

        let filename = name.lowercased()
        if filename == "dockerfile" { return "shippingbox" }
        if filename == "makefile" { return "hammer" }
        if filename == ".gitignore" || filename == ".env" { return "doc.badge.gearshape" }
        ...
        case "bash", "command", "fish", "sh", "zsh":
            return "terminal"
        case "htm", "html", "svelte", "vue", "xhtml":
            return "globe"
        ...
        case "db", "sqlite", "sqlite3", "sql":
            return "cylinder"
        case "bmp", "gif", "heic", "icns", "jpeg", "jpg", "png", "tiff", "webp":
            return "photo"
        ...
        case "7z", "bz2", "gz", "rar", "tar", "tgz", "xz", "zip":
            return "archivebox"
        case "json", "jsonc", "plist", "toml", "xml", "yaml", "yml":
            return "curlybraces"
        case "log", "md", "markdown", "pdf", "rtf", "txt":
            return "doc.plaintext"
        case "c", "cc", "cpp", "cs", "go", "h", "hpp", "java", "js", "jsx",
             "kt", "lua", "m", "mm", "php", "py", "rb", "rs", "ts", "tsx":
...
struct FileViewerView: View {
    ...
    var body: some View {
        ...
        // The selected file view stays mounted while Sora is inactive, so
        ...
        // inactive (for example when moving between Sora windows).
        .onReceive(NotificationCenter.default.publisher(
