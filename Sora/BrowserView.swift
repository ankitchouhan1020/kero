...
private enum BrowserContextMenuSupport {
    static let messageName = "soraContextMenuLink"
    static let script = #"""
    (() => {
      window.addEventListener("contextmenu", event => {
        ...
        window.webkit.messageHandlers.soraContextMenuLink.postMessage(
          link?.href || ""
        );
      }, true);
    })()
...
final class BrowserWebView: WKWebView {
    ...
    private static let openLinkInNewTabIdentifier =
        NSUserInterfaceItemIdentifier("sora.browser.openLinkInNewTab")
    private static let openLinkInNewPaneIdentifier =
        NSUserInterfaceItemIdentifier("sora.browser.openLinkInNewPane")
...
final class BrowserTab: NSObject, ObservableObject, Identifiable, WKNavigationDelegate, WKUIDelegate {
    ...
    /// Keep target=_blank links inside this browser tab. Sora has an explicit
...
/// so it follows Sora's appearance while the page remains a real WKWebView.
