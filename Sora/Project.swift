...
/// the project open but empty — only the explicit "Remove Space" action (see
...
final class Project: nonisolated ObservableObject, nonisolated Identifiable {
    ...
    /// Pins or unpins a tab and moves it to the lifecycle boundary. Keeping
    /// the canonical array partitioned makes pointer order, keyboard order,
    /// close-below, persistence and automation all describe the same list.
    func setPinned(_ pinned: Bool, tabID: UUID) {
        ...
        tab.isPinned = pinned
        tabs.insert(tab, at: tabs.firstIndex(where: { !$0.isPinned }) ?? tabs.endIndex)
        assertLifecycleOrder()
    }

    /// Moves a tab across another tab in visible order. Crossing the section
    /// boundary explicitly adopts the target's lifecycle before reordering.
    func moveTab(_ draggedID: UUID, to targetID: UUID) {
        guard draggedID != targetID,
              let source = tabs.firstIndex(where: { $0.id == draggedID }),
              let originalTarget = tabs.firstIndex(where: { $0.id == targetID })
        else { return }
        let movesForward = source < originalTarget
        let tab = tabs.remove(at: source)
        tab.isPinned = tabs.first(where: { $0.id == targetID })?.isPinned ?? tab.isPinned
        guard let target = tabs.firstIndex(where: { $0.id == targetID }) else { return }
        tabs.insert(tab, at: movesForward ? target + 1 : target)
        assertLifecycleOrder()
    }

    /// Repairs pre-Spaces snapshots whose pinned and temporary tabs were
    /// interleaved while preserving relative order inside each section.
    func normalizeTabOrder() {
        let ordered = tabs.filter(\.isPinned) + tabs.filter { !$0.isPinned }
        if ordered.map(\.id) != tabs.map(\.id) { tabs = ordered }
        assertLifecycleOrder()
    }

    private func assertLifecycleOrder() {
        var reachedTemporary = false
        for tab in tabs {
            if !tab.isPinned { reachedTemporary = true }
            assert(!reachedTemporary || !tab.isPinned, "Pinned tab followed a temporary tab")
        }
    }

    /// Moves a dragged tab across a target in the same visible category:
    /// after it when moving right, or before it when moving left. Selection
    /// continues to follow its tab ID.
    func moveTabWithinCategory(_ draggedID: UUID, to targetID: UUID) {
        TabCategory.reorder(
            &tabs,
            moving: draggedID,
            to: targetID,
    ...
    func restoreTab(
        ...
        let containsTerminal = columns.flatMap(\.panes).contains {
            if case .session = $0.content { return true }
            return false
        }
        let tab = PaneTab(
            columns: columns,
            focusedPaneID: columns[col].panes[row].id,
            isPinned: snap.isPinned ?? containsTerminal
        )
    ...
    /// Inserts beside the selection when both tabs share a lifecycle. Across
    /// sections, insert at the pinned/temporary boundary so the canonical
    /// array always matches the vertical sidebar.
    private func insertNextToSelected(_ tab: PaneTab) {
        if let selectedTabID,
           let index = tabs.firstIndex(where: { $0.id == selectedTabID }),
           tabs[index].isPinned == tab.isPinned {
