...
final class TerminalManager: nonisolated ObservableObject {
    ...
    /// Finder service requests use it to target the active Sora window.
    ...
    /// Folder requests can arrive while macOS is still launching Sora, before
    ...
    /// reopen Sora after the user has closed its last window.
    ...
    func presentSpaceCreator() {
        isSpaceCreatorPresented = true
    }

    @discardableResult
    func createSpace(name: String, icon: String?, repositories: [String]) -> Project {
        let project = makeProject(createInitialSession: false)
        project.customName = name.trimmingCharacters(in: .whitespacesAndNewlines)
        project.customIcon = icon
        project.repositories = repositories
        if repositories.isEmpty {
            project.newSession()
        } else {
            for repository in repositories {
                project.newSession(directory: repository)
                project.selectedTab?.customName = URL(
    ...
    func newProject(directory: String, name: String? = nil) -> Project {
        ...
        project.repositories = [directory]
    ...
    /// Routes folders from the Finder service into the active Sora window.
    ...
    /// Commands are constructed during app launch even when macOS starts Sora
    ...
    func close(_ project: Project) {
        ...
        alert.messageText = "Remove Space “\(project.name)”?"
        alert.informativeText = "This will close every tab and terminal session in this Space. Unsaved file changes will be lost. This can’t be undone."
        let remove = alert.addButton(withTitle: "Remove")
        remove.keyEquivalent = "\r"
        let cancel = alert.addButton(withTitle: "Cancel")
        cancel.keyEquivalent = "\u{1b}"

        let finish: (NSApplication.ModalResponse) -> Void = { [weak self, weak project] response in
            guard response == .alertFirstButtonReturn,
                  let self, let project,
                  self.projects.contains(where: { $0.id == project.id })
            else { return }
            self.removeSpace(project)
        }
        if let window {
    ...
    /// Removes a Space after its caller has handled destructive confirmation.
    func removeSpace(_ project: Project) {
    ...
    private func remove(_ project: Project) {
        ...
        if selectedProjectID == project.id {
            selectedProjectID = projects.isEmpty
                ? nil
                : projects[max(0, index - 1)].id
        }
        // Nothing left to inspect once the last Space is gone, so collapse
        // the right sidebar and immediately offer a fresh start.
        if projects.isEmpty {
    ...
    /// Opens a browser tab in the current project. Sora remains
    ...
    /// Activates Sora and reveals the session that emitted a desktop
            ...
            projects: projects.compactMap { project in
                ...
                let tabs = project.tabs.map { tab -> ProjectSnapshot.TabSnapshot in
                    ...
                    return ProjectSnapshot.TabSnapshot(
                        ...
                        customName: tab.customName, isPinned: tab.isPinned
                    )
                }
    ...
    private func restore(from snapshot: SessionSnapshot) -> Bool {
        ...
        for saved in snapshot.projects where !saved.tabs.isEmpty {
            ...
            project.customIcon = saved.customIcon
            project.customDirectory = saved.customDirectory
            project.repositories = saved.repositories ?? saved.customDirectory.map { [$0] } ?? []
