...
SwiftPM package so sora can link `tree_sitter_tsx()`.
...
sora's grammars otherwise come from `STTextView-Plugin-Neon`, which *does*
...
byte-identical to TypeScript's, which sora already reaches through
...
`Sources/TreeSitterTSX/` — the same revision sora pins in
`Sora.xcodeproj/project.pbxproj`, so the grammar matches the queries and the
